...
{
    public class RobotPathConfigureUpdateHandler : AbstractWebAPIMessageHandlercs
    {
        public WebSocketMessage Execute(string userName, string clientip, string type, bc_robot_path_configure data)
        {
            ...
            try
            {
                ...
                switch (type)
                {
                    case "RobotPathConfigureAdd":
                        {
                            dbService.Insertbc_robot_path_configure(data);
                        }
                        break;
                    case "RobotPathConfigureUpdate":
                        {
                            dbService.Updatebc_robot_path_configure(data);
                        }
                        break;
                    default:
                        break;
