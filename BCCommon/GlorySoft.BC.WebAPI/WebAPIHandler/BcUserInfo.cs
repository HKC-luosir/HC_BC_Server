...
{
    public class BcUserInfo : AbstractWebAPIMessageHandlercs
    {
        public WebSocketMessage Execute(UserInfoRequest userInfoRequest)
        {
            ...
            try
            {
                ...
                if (userInfoRequest.userId != "")
                {
                    userList = userList.Where(t => t.UserID == userInfoRequest.userId).ToList();
                }
                if (userInfoRequest.userName != "")
                {
                    userList = userList.Where(t => t.UserID == userInfoRequest.userName).ToList();
                }
