...
{
    public class GetCfgMixRunInputRatioUpdateHandler : AbstractWebAPIMessageHandlercs
    {
        public WebSocketMessage Execute(string userName, string clientip, string type, MIXRunInputRatio Init)
        {
            ...
            try
            {
                ...
                hashtable.Add("InputRatioID", Init.InputRatioID);
                switch (type)
                {
                    case "GetCfgMixRunInputRatioUpdate":
                        body = dbService.UpdateMIXRunInputRatio(Init) > 0 ? true : false;
                        break;
                    case "GetCfgMixRunInputRatioAdd":

                        if (dbService.GetMIXRunInputRatioList(hashtable).Count > 0)
                        {
                            break;
                        }
                        body = dbService.InsertMIXRunInputRatio(Init);
                        break;
                    case "GetCfgMixRunInputRatioDelete":
                        body = dbService.DeleteMIXRunInputRatio(hashtable) > 0 ? true : false;
                        break;
                    default:
                        break;
                }
