...
{
    public class SamplingFlagReportBlockHandler : AbstractEventHandler
    {
        public SamplingFlagReportBlockHandler(IPLCContext context)
            : base(context)
        {
        }
        public override void Execute(PLCEventArgs args)
        {
            try
            {
                ...
                if (oEQP == null)
                {
                    LogHelper.EIPLog.ErrorFormat("+++ SamplingFlagReportBlockHandler:{0} Cannot Find EQPInfo +++", eqpName);
                    return;
                }
                ...
                logicService.SamplingFlagReport(oEQP, jobID, lotSequenceNumber, slotSequenceNumber, txid);
            }
            catch (Exception ex)
            {
                LogHelper.EIPLog.ErrorFormat("+++ SamplingFlagReportBlockHandler:{0} ,Error:{1} +++", args.Message.EQPName, ex.ToString());
            }
        }
    }
}