...
{
    public class RobotArmSubstrateLoadReportBlockHandler : AbstractEventHandler
    {
        public RobotArmSubstrateLoadReportBlockHandler(IPLCContext context)
            : base(context)
        {
        }
        public override void Execute(PLCEventArgs args)
        {
            try
            {
                ...
                if (oEQP == null)
                {
                    LogHelper.EIPLog.ErrorFormat("+++ RobotArmSubstrateLoadReportBlockHandler:{0} Cannot Find EQPInfo +++", args.Message.EQPName);
                    return;
                }
