...
{
    public class CSTMoveOutReportBlockHandler : AbstractEventHandler
    {
        public CSTMoveOutReportBlockHandler(IPLCContext context)
            : base(context)
        {
        }
        public override void Execute(PLCEventArgs args)
        {
            try
            {
                ...
                if (oEQP == null)
                {
                    LogHelper.EIPLog.ErrorFormat("+++ CSTMoveOutReportBlockHandler:{0} Cannot Find EQPInfo +++", eqpName);
                    return;
                }
            ...
            {
                LogHelper.EIPLog.ErrorFormat("+++ CSTMoveOutReportBlockHandler:{0} ,Error:{1} +++", args.Message.EQPName, ex.ToString());
            }
        }
    }
}