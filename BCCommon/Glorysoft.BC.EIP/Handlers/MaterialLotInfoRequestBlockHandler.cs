...
namespace Glorysoft.BC.EIP.Handlers
{
    ...
    public class MaterialLotInfoRequestBlockHandler : AbstractEventHandler
    {
        public MaterialLotInfoRequestBlockHandler(IPLCContext context)
           : base(context)
        {
        }
        ...
        public override void Execute(PLCEventArgs args)
        {
            try
            {
                ...
                if (oEQP == null)
                {
                    LogHelper.EIPLog.ErrorFormat("+++ MaterialLotInfoRequestBlockHandler:{0} Cannot Find EQPInfo +++", args.Message.EQPName);
                    return;
                }
                ...
                logicService.MaterialLotInfoRequest(oEQP, i, materialID, unitNumber, slotNumber, materialType, txid);
            }
            catch (Exception ex)
            {
                LogHelper.EIPLog.ErrorFormat("+++ MaterialLotInfoRequestBlockHandler:{0} ,Error:{1} +++", args.Message.EQPName, ex.ToString());
            }
        }
    }
}
