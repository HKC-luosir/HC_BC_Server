...
{
    public class RobotArmMonitoringBlockHandler : AbstractEventHandler
    {
        public RobotArmMonitoringBlockHandler(IPLCContext context)
            : base(context)
        {
        }
        public override void Execute(PLCEventArgs args)
        {
            try
            {
                ...
                if (oEQP == null)
                {
                    LogHelper.EIPLog.ErrorFormat("+++ RobotArmMonitoringBlockHandler:{0} Cannot Find EQPInfo +++", args.Message.EQPName);
                    return;
                }
