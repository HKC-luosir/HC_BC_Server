...
{
    public class RobotCommandBufferMonitoringBlockHandler : AbstractEventHandler
    {
        public RobotCommandBufferMonitoringBlockHandler(IPLCContext context)
            : base(context)
        {
        }
        public override void Execute(PLCEventArgs args)
        {
            try
            {
                ...
                if (oEQP == null)
                {
                    LogHelper.EIPLog.ErrorFormat("+++ RobotCommandBufferMonitoringBlockHandler:{0} Cannot Find EQPInfo +++", args.Message.EQPName);
                    return;
                }
